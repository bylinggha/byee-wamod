...
export const makeSocket = (config) => {
    ...
    const sendRawMessage = async (data) => {
        // ✅ FIXED: Double check untuk prevent race condition
        if (!ws.isOpen || ws.isClosed || ws.isClosing) {
        ...
        await promiseTimeout(connectTimeoutMs, async (resolve, reject) => {
            // ✅ FIXED: Check lagi sebelum send untuk prevent race condition
            if (!ws.isOpen || ws.isClosed || ws.isClosing) {
                reject(new Boom('Connection Closed', { 
                    statusCode: DisconnectReason.connectionClosed
                }));
                return;
            }
    ...
    const requestPairingCode = async (phoneNumber, customPairingCode = "BAILLITE") => {
