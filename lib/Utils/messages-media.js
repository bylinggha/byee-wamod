...
// ✅ SMART: Cleanup expired cache entries setiap 10 menit
setInterval(() => {
    const now = Date.now();
...
export const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (filePath, { mediaType, fileEncSha256B64, timeoutMs }) => {
        ...
        // ✅ SMART: Retry refreshMediaConn jika hosts kosong (max 3 attempts)
        let uploadInfo = await refreshMediaConn(false);
        let retryCount = 0;
        const maxRetries = 3;
        
        while ((!uploadInfo.hosts || uploadInfo.hosts.length === 0) && retryCount < maxRetries) {
            retryCount++;
            logger.warn(`No hosts in media connection, retrying (${retryCount}/${maxRetries})...`);
