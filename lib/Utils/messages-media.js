...
export async function generateThumbnail(file, mediaType, options) {
    ...
    if (mediaType === 'image') {
        ...
        // ✅ DEBUG: Gunakan console.error (tidak di-disable) untuk debugging
        console.error(`🔍 [THUMBNAIL] Generating thumbnail 192px from file: ${file}`);
        const { buffer, original } = await extractImageThumb(file, 192);
        thumbnail = buffer.toString('base64');
        console.error(`✅ [THUMBNAIL] Thumbnail generated: ${thumbnail.length} chars (base64), original size: ${original?.width}x${original?.height}`);
