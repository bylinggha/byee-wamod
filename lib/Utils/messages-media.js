...
export const extractImageThumb = async (bufferOrFilePath, width = 32) => {
    ...
    try {
        ...
        // ✅ SMART: Check jika file path dan file tidak ada
        if (typeof bufferOrFilePath === 'string') {
            try {
                await fs.access(bufferOrFilePath);
            } catch (accessError) {
                // ✅ File tidak ada - throw error yang jelas untuk di-handle oleh caller
                throw new Boom(`File not found: ${bufferOrFilePath}`);
            }
        }
        ...
        if ('sharp' in lib && typeof lib.sharp?.default === 'function') {
            try {
                ...
                // ✅ SMART: Jika Sharp gagal (file corrupt/tidak ada), fallback ke Jimp
                const errorMsg = sharpError.message || '';
                if (errorMsg.includes('unable to open') || errorMsg.includes('No such file') || errorMsg.includes('corrupt header')) {
                    throw new Boom(`File not accessible or corrupt: ${bufferOrFilePath}`);
                }
                throw sharpError;
            }
        }
        ...
        if (typeof JimpClass === 'function' && typeof JimpClass.read === 'function') {
            ...
            // ✅ FIX: Jimp modern (0.16.x) menggunakan format object untuk resize
            // ✅ Format: resize({ w: width, h: JimpClass.AUTO }) atau resize(width) untuk auto height
            // ✅ FIX: Jimp modern format - gunakan { w: width } saja (auto height)
            // ✅ Jimp.AUTO = -1, tapi lebih aman pakai format tanpa h untuk auto-calculate
