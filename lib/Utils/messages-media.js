    ...
    return async (filePath, { mediaType, fileEncSha256B64, timeoutMs }) => {
        ...
        for (let i = 0; i < hosts.length; i++) {
            ...
            try {
                ...
                // ✅ CRITICAL FIX: Check response text SEBELUM parse JSON untuk detect "error code: 502"
                // ✅ Response bisa status 200 OK tapi body-nya "error code: 502" (bukan JSON)
                ...
                if (contentType.includes('application/json')) {
                    try {
                        // ✅ CRITICAL FIX: Parse dari text yang sudah diambil (jangan call response.json() lagi)
                        parsed = JSON.parse(responseText);
                    } catch (jsonError) {
                        // ✅ SMART: Jika JSON parse gagal, log detail error
                        logger.warn({ 
                            jsonError: jsonError.message, 
                            jsonErrorCode: jsonError.code,
                        ...
                        // ✅ CRITICAL FIX: Provide more detailed error message
                        throw new Error(`Invalid JSON response: ${jsonError.message} (Code: ${jsonError.code || 'N/A'}, Length: ${responseText.length}). Raw response: ${responseText.substring(0, 100)}`);
                    }
                } else {
